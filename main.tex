...
\\\\
Działanie pojedynczej iteracji algorytmu można zaimplementować w następujący sposób:

\begin{lstlisting}
TODO
\end{lstlisting}

Końcowa struktura pojedynczego punktu wygląda zatem następująco:
...
Metoda opisana w punkcie 2.2.1 została w zaimplementowana w całości, z uwzględnieniem dwóch warunków defniowania punktu jako odstający. Wybór jednej z nich odbywa się na żądanie użytkownika. W procesie testowania metod oba warunki są używane w celu porównania ich działania.\\
Końcowe działanie metody można przedstawić na prsotym przykładzie:
\begin{itemize}
	\item badaniu został poddany zbiór zawierający 30 punktów. Liczba klastrów została ustalona na 2. Podział wygląda następująco:
	\item zostały obliczone odległości od centroidów klastrów a następnie obliczone potrzebne miary:
	\item w zależności od wyboru metody punkty sklasyfikowane jako odstające po pierwszej iteracji zostały zaznaczone na czerwono:
\end{itemize}
\subsection{Metoda ilości punktów w klastrze z wykorzystaniem algorytmu KMean}
Metoda opisana w punkcie 2.2.2 została zaimplementowana zgodnie z opisem uwzględniając obliczenie paramaterów, które są wymagane do obiczenia warunku (progu):
\begin{itemize}
	\item stosunek wielkości zbioru danych do ilości klastrów
	\item ilość punktów w każðym klastrze
	\item średniej liczby punktów na klaster
\end{itemize}
Znając powyższe parametry oraz zadany z góry warunek (próg) można zdefniować równanie, na podstawie którego rozpatrywane są pojedyncze klastry:
\\
\begin{figure}[H]
  \begin{center}
  \includegraphics[scale=0.7]{linear_reg}
...
Implementacja w kodzie może wyglądać następująco:
...
\section{Prygotowanie środowiska}
...
\\\\
Przed przystąpieniem do testów zostały wybrane tylko te zbiory danych, które zawierają więcej niż 800 pojedynczcyh punktów (dane z conajmniej dwóch lat). Dzięki temu wszystkie wybrane algorytmy powinny działać na takich samych zasadach (odpowiednia ilość punktów w klastrach a trend, jeśli istnieje, został już wyznaczony, możliwość dobrego podziału na okna czasowe).
...
\textbf{Metoda odległości od centroidu (3.3.4)}
\begin{itemize}
	\item ilość klastrów: 20, 40
	\item wszystkie trzy dodatkowe cechy (3.3.1)
...
\\\\
\textbf{Metoda ilości punktów w klastrze (3.3.5)}
\begin{itemize}
	\item ilość klastrów: 20, 40
	\item wszystkie trzy dodatkowe cechy (3.3.1)
...
\\\\
\textbf{Metoda regresji liniowej z oknem czasowym (3.3.6)}
\begin{itemize}
	\item wszystkie trzy dodatkowe cechy (3.3.1)
...
Ważną częęcią etapu uruchamiania testów i badań jest stworzenie programu, który w sposo” automatyczny będzie w stanie uruchomić wcześniej zdefinowane metody kolejno z różnymi parametrami. WYnika to z tego, że cały proces uruchamiania metod jest czasochłonny. Uruchamianie kolejno metod ręcznie jest możliwe, jednak nieoptymalne (brane jest pod uwagę ręczne uruchomienie pojedynczych testów tylko w momencie chęci potwierdzenia wyniku). 
...
Po przeprowadzeniu wzystkich zdefiniowanych testów zostanie uruchomiona procedura tworzenia raportów. Pojedynczy raport tyczy się jednego zbioru danych oraz metod jakie zostały na nim uruchomione. W skład tej procedury wchodzą:
\begin{itemize}
